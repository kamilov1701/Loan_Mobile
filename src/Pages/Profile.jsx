...
function Profile() {
    ...
    useEffect(() => {
        ...
        const unsubscribe = onSnapshot(collection(db, "loaners"), (snapshot) => {
            const allLoaners = snapshot.docs
                .map(doc => ({ id: doc.id, ...doc.data() }))
                .filter(l => l.ownerId === auth.currentUser.uid); // filter by owner
    ...
    return (
        <div className="relative">
            ...
            <button
                onClick={handleLogout}
                className="absolute top-[20px] right-[20px] text-red-600 bg-[#FFEAEA] px-[12px] py-[8px] rounded-full flex items-center gap-[5px] z-50"
            >
                <FiLogOut size={18} /> Chiqish
            </button>
            ...
            <section className="px-[22px] mt-[30px]">
                <div>
                    <div className="text-center flex flex-col items-center">
                        <img className="rounded-[50px]" src={Profile_Logo} alt="" />
                            ...
                            <>
                                ...
                                <div className="mt-[20px]">
                                    {filteredPaid.map(item => (
                                        ...
                                        >
                                            <h3 className="font-bold text-[14px]">{item.fullName}</h3>
                                            <p className="text-[12px] text-[#6B7280]">{item.mainPhone || item.phone || "No phone"}</p>
                                        </div>
                                    ))}
